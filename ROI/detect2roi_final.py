...
# -------------------- [설정] --------------------
VIDEO_SRC    = 'demo_video.mp4'   # '0' → 웹캠, 그 외는 파일 경로
MODEL_PATH   = 'yolov8n-pose.pt'
ROI_JSON     = 'seat_roi.json'
CONF         = 0.3
SERVICE_KEY  = 'hahaha-79a4a-77a0d09dc1d4.json'
HOLD_SEC     = 1.0
TARGET_FPS   = 30  # 초당 목표 프레임 수 (실시간 동기화용)
...
def write_occupancy_to_firestore(occ: dict):
    ...
    - 추가: isOccupied 가 True->False 로 바뀌는 "하강 에지"에서 reserved 를 False 로 자동 해제
    """
    if not _FIREBASE_OK:
        return
    ...
    try:
        for sid_raw, taken in occ.items():
            ...
            # 하강 에지에서 예약 해제 (앱의 정교한 로직과 충돌하여 주석 처리)
            # if key in falling_keys:
            #     data["reserved"] = False
            #     # 필요 시 예약자도 정리하고 싶다면 아래 주석 해제:
...
def compute_occupancy(img, model, rois, seat_centers, conf=0.3):
    res = model(img, conf=conf, verbose=False, device=0)[0]  # GPU 사용으로 변경
...
def main():
    ...
    src = VIDEO_SRC
    delay_between_frames = 1 / TARGET_FPS
    ...
    if src == '0':
        ...
        while True:
            ...
            wait_time = delay_between_frames - elapsed
            if wait_time > 0:
    ...
    if is_image(src):
        ...
        while True:
            ...
            else:                          t = (fno - 1) / (1/delay_between_frames)
